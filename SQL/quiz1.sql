...
--coalesce 활용
--coalesce 함수는 컬럼1이 null이 아니면 컬럼1 값을 리턴, 컬럼1이 null이고 컬럼2가 null이 아니면 컬럼2를 리턴
--그래서 한마디로 처음으로 null이 아닌 컬럼값을 만나면 그 컬럼값을 리턴한다.
--- coalesce(productdescription, 'None') 이렇게 쓰면 productdescription 컬럼 중 null값이 나오면 None이라는 문자를 
select *, coalesce(productdescription, 'None') as new_productdescription
from products
